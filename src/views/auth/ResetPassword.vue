  ...
  <div class="login-container">
    ...
    <div class="login-box">
      ...
      <div class="login-header">
        ...
        <p class="login-subtitle">Đặt lại mật khẩu</p>
      </div>

      <!-- Thiếu token -->
      <div v-if="!token" class="error-state">
        <el-icon class="error-icon"><WarningFilled /></el-icon>
        <p class="error-title">Link không hợp lệ</p>
        <p class="error-desc">Link đặt lại mật khẩu không hợp lệ hoặc đã hết hạn.</p>
        <router-link :to="{ name: 'ForgotPassword' }">
          <el-button class="login-button" type="primary" size="large">Yêu cầu link mới</el-button>
        </router-link>
      </div>
      ...
      <div v-else-if="success" class="success-state">
        ...
        <p class="success-title">Đặt lại thành công!</p>
        <p class="success-desc">Mật khẩu của bạn đã được thay đổi. Vui lòng đăng nhập lại.</p>
        <router-link :to="{ name: 'Login' }">
          <el-button class="login-button" type="primary" size="large">Đăng nhập ngay</el-button>
        </router-link>
      </div>
...
import { Lock, Message, CircleCheck, WarningFilled } from '@element-plus/icons-vue'
...
// Lấy token và email từ query string: /reset-password?token=xxx&email=xxx
const token = ref('')
const form = reactive({
  email: '',
  password: '',
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        ElMessage.error(error.response?.data?.message || 'Đặt lại mật khẩu thất bại. Vui lòng thử lại.')
      } finally {
        loading.value = false
      }
    }
...
.logo-wrap { display: flex; align-items: center; justify-content: center; gap: 12px; margin-bottom: 10px; svg { filter: drop-shadow(0 0 12px rgba(56,189,248,0.6)); } }
...
.login-form {
  ...
  :deep(.el-input__wrapper) { background: rgba(12,25,41,0.6) !important; box-shadow: 0 0 0 1px rgba(56,189,248,0.2) inset !important; border-radius: 10px; padding: 10px 14px; transition: all 0.25s ease; &:hover { box-shadow: 0 0 0 1px rgba(56,189,248,0.4) inset !important; } &.is-focus { box-shadow: 0 0 0 2px rgba(56,189,248,0.45) inset !important; } }
...
.login-button {
  ...
  &:hover { background: linear-gradient(135deg, #38bdf8 0%, #0ea5e9 100%) !important; transform: translateY(-2px); }
}

// Success / Error states
.success-state, .error-state { text-align: center; padding: 8px 0; }
.success-icon { font-size: 56px; color: #34d399; margin-bottom: 16px; filter: drop-shadow(0 0 16px rgba(52,211,153,0.4)); }
.error-icon { font-size: 56px; color: #f87171; margin-bottom: 16px; }
.success-title, .error-title { font-size: 18px; font-weight: 700; color: #e0f4ff; margin: 0 0 10px; }
.success-desc, .error-desc { font-size: 13.5px; color: rgba(186,220,248,0.65); line-height: 1.6; margin-bottom: 24px; }
...
.login-footer { text-align: center; margin-top: 24px; font-size: 12px; color: rgba(148,200,235,0.35); }
